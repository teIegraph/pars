...
async def process_price_for_slug(gift_slug, text):
    """Общая логика: извлекаем цену и отправляем в группу."""
    price = extract_price(text)
    if price is None:
        print(f'[!] Цена не найдена для {gift_slug}: {text[:200]}', flush=True)
        return
...
async def price_bot_handler(event):
    ...
    # Случай 1: пришли кнопки — жмём "Gift information"
    if msg_id in pending_price_requests and buttons:
        gift_slug = pending_price_requests.pop(msg_id)
        print(f'[PRICE_BOT] Кнопки получены для {gift_slug}', flush=True)
        target_button = None
        for row in buttons:
            for btn in row:
                btn_text = (btn.text or '').lower()
                # Ищем "Gift information" (англ) или "Информация о подарке" (рус)
                if ('gift' in btn_text and 'info' in btn_text) or \
                   ('информация' in btn_text and 'подар' in btn_text):
                    target_button = btn
                    break
            if target_button:
                break
        if not target_button:
            print(f'[!] Кнопка "Gift information" не найдена для {gift_slug}', flush=True)
            print(f'    Доступные: {[b.text for row in buttons for b in row]}', flush=True)
            return
        try:
            print(f'[PRICE_BOT] Жму кнопку: {target_button.text}', flush=True)
            await target_button.click()
            # Ответ может прийти отдельным сообщением — запоминаем связку
            pending_price_reply[msg_id] = gift_slug
            # Ждём 5 секунд и пытаемся поймать ответ
            await asyncio.sleep(5)
            # Если ответ ещё не пришёл, оставляем связку — поймает следующий handler
        except Exception as e:
            print(f'[!] Ошибка клика: {e}', flush=True)
        return

    # Случай 2: пришёл ответ с ценой (после клика)
    if msg_id in pending_price_reply:
        gift_slug = pending_price_reply.pop(msg_id)
        await process_price_for_slug(gift_slug, text)
        return

    # Случай 3: любое другое сообщение от PriceNFTbot с ценой — пытаемся связать с последним запросом
    if text and extract_price(text):
        # Если есть хоть один pending — забираем
        if pending_price_reply:
            # Берём самый старый pending
            old_id = next(iter(pending_price_reply))
            gift_slug = pending_price_reply.pop(old_id)
            print(f'[PRICE_BOT] Связываю ответ с {gift_slug}', flush=True)
            await process_price_for_slug(gift_slug, text)
