...
# Смотрим только последние N подарков (новые всегда сверху)
...
async def price_bot_handler(event):
    ...
    if event.message.reply_to_msg_id in pending_price_requests:
        ...
        if not text:
            ...
            return
        print(f'[PRICE_BOT] Ответ для {gift_slug}: {text[:300]}', flush=True)
...
def extract_slug_from_gift(gift):
    """Достаём slug из объекта подарка."""
...
async def fetch_recent_gifts(username, limit=MAX_GIFTS_PER_CHECK):
    """Забираем только первые N подарков (самые новые)."""
...
async def initialize_seen():
    """Запоминаем только последние N подарков."""
    global initialized
